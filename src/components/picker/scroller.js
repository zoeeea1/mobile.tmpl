...
const passiveSupported = require('../popup/passive_supported')
...
var Scroller = function (container, options) {
  ...
  if (data.length && data[0].constructor === Object) {
    data.forEach(function (row) {
      ...
      html += '<div class="' + self.options.itemClass + '" data-name=' + JSON.stringify({ name: row.Name }) + '>' + row.Name + '</div>'
    })
  } else {
    data.forEach(function (val) {
      html += '<div class="' + self.options.itemClass + '" data-name=' + JSON.stringify({ name: val }) + '>' + val + '</div>'
    })
  }
...
var members = {
  ...
  select(value, animate) {
    ...
    for (var i = 0, len = children.length; i < len; i++) {
      if (decodeURI(JSON.parse(children[i].dataset.name).name) === value) {
  ...
  __selectItem(selectedItem) {
    ...
    if (self.name !== null) {
      self.__prevValue = self.name
    }

    self.name = decodeURI(JSON.parse(selectedItem.dataset.name).name)
  },
  ...
  __scrollingComplete() {
    ...
    if (self.__prevValue !== null && self.__prevValue !== self.name) {
      self.options.onSelect(self.name)
    }
  },
